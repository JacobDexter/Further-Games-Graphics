...
HRESULT Application::Initialise(HINSTANCE hInstance, int nCmdShow)
{
    ...
    //camera
    camera.push_back(new Camera(XMFLOAT3(0.0f, 0.0f, -3.0f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), (float)_WindowHeight, (float)_WindowWidth, 0.01f, 1000.0f, 0.0f));
    camera.push_back(new Camera(XMFLOAT3(0.0f, 0.0f, 100.0f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), (float)_WindowHeight, (float)_WindowWidth, 0.01f, 1000.0f, 180.0f));
    camera.push_back(new Camera(XMFLOAT3(0.0f, 100.0f, 50.0f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), (float)_WindowHeight, (float)_WindowWidth, 0.01f, 1000.0f, 90.0f));
...
void Application::Update()
{
    ...
    // Animate the cube
    //
	XMStoreFloat4x4(&_world, XMMatrixScaling(30.0f, 30.0f, 30.0f) * XMMatrixRotationY(t * 2.0f) * XMMatrixRotationX(t * 2.0f) * XMMatrixTranslation(0.0f, 0.0f, 50.0f));

    //spin
    XMStoreFloat4x4(&_world2, XMMatrixRotationY(rotationPlayer) * XMMatrixTranslation(xPlayer, yPlayer, zPlayer));

    //cam
    camera[selectedCamera]->Update(t);
}

void Application::Input(float t)
{
    ...
    //move camera along z and x axis
    ...
    {
        rotationPlayer = t * 0.5f;
    }
    if (GetAsyncKeyState(VK_RIGHT)) //right arrow
    {
        rotationPlayer = t * -0.5f;
    }
    if (GetAsyncKeyState(VK_UP)) //up arrow (reset rotation)
    {
        rotationPlayer = 0.0f;
    }
}
